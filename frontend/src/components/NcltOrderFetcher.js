...
export default function NcltOrderFetcher({ caseRecord = null }) {
  ...
  const start = async event => {
    ...
    try {
      ...
      if (data.status === "MANUAL_ACTION_REQUIRED") toast.warning("The portal displayed an explicit human-verification challenge. Complete it in the opened browser, then continue.", { duration: 12000 });
    } catch (error) {
  ...
  const continueRun = async () => {
    ...
    try {
      ...
      setRun(data); await loadHistory();
      if (data.status === "MANUAL_ACTION_REQUIRED") toast.warning("The explicit human-verification challenge is still present.");
  ...
  return <div className="space-y-6 nclt-fetcher" data-testid="nclt-order-fetcher">
    <section className="casefile-panel nclt-fetch-form">
      ...
      <p className="module-description">Enter the case number and year. Casefile searches the public NCLT e-filing Case History page, retrieves the proceeding history, and identifies every available order.</p>
      <form className="form-grid" onSubmit={start}>
        ...
        <div className="full nclt-form-actions"><button className="primary-button" disabled={busy || !config}><FileSearch size={16} />{busy ? "Checking NCLT…" : "Fetch NCLT case"}</button>{run && <button type="button" className="outline-button" onClick={() => setRun(null)}><RefreshCw size={15} />New check</button>}</div>
      </form>
    </section>

    {run?.status === "MANUAL_ACTION_REQUIRED" && <section className="casefile-panel nclt-manual"><AlertTriangle size={24} /><div><h3>Portal human verification detected</h3><p>{run.error_message}</p><ol><li>Switch to the opened browser window.</li><li>Complete only the challenge displayed by the NCLT portal.</li><li>Return here and press Continue.</li></ol><button className="primary-button" disabled={busy} onClick={continueRun}>{busy ? "Checking…" : "Continue after verification"}</button></div></section>}
