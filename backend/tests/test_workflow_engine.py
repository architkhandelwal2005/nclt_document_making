...
def test_master_seed_is_versioned_and_case_execution_is_separate(tmp_path):
    ...
    assert [item["step_code"] for item in definitions] == [f"CIRP-{number:03d}" for number in range(1, 85)]
...
def test_event_workflow_task_and_deadline_creation_are_idempotent(tmp_path):
    ...
    with store.connect() as connection:
        ...
        assert connection.execute("SELECT COUNT(*) FROM case_workflow_steps WHERE case_id=?", (case["id"],)).fetchone()[0] == 84
        assert connection.execute("SELECT COUNT(*) FROM case_deadlines WHERE case_id=?", (case["id"],)).fetchone()[0] == 84
...
def test_workflow_summary_and_event_ledger_api(tmp_path, monkeypatch):
    ...
    with TestClient(server.app) as client:
        ...
        assert payload["total_steps"] == 84
        assert payload["ready"] == 21
        assert payload["not_triggered"] == 63
