...
def test_event_workflow_task_and_deadline_creation_are_idempotent(tmp_path):
    ...
    with store.connect() as connection:
        ...
        assert connection.execute("SELECT COUNT(*) FROM case_workflow_steps WHERE case_id=?", (case["id"],)).fetchone()[0] == 56
        assert connection.execute("SELECT COUNT(*) FROM case_deadlines WHERE case_id=?", (case["id"],)).fetchone()[0] == 56
...
def test_workflow_summary_and_event_ledger_api(tmp_path, monkeypatch):
    ...
    with TestClient(server.app) as client:
        ...
        assert payload["total_steps"] == 56
        assert payload["ready"] == 21
        assert payload["not_triggered"] == 35
